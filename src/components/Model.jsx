import { Html, MeshTransmissionMaterial, PerspectiveCamera, useAnimations, useEnvironment, useGLTF, useScroll, useVideoTexture } from '@react-three/drei'
...
export default function Model(props) {
  ...
  useLayoutEffect(() => {
    tl.current = gsap.timeline({ defaults: { duration: 0.1, ease: 'spring ' }, paused: true })
  ...
  return (
    <group ref={myref} {...props} dispose={null} >
      <group name="Scene">
        <mesh ref={meshRef} position={[-4, -2.3,-1.7]} rotation={[0,3.2,0]} scale={2.5}>
          <group ref={manRef} name="Armature002" position={[0, 2, -4.15]} rotation={[Math.PI / 2,0, 0]} scale={0}>
            <primitive object={nodes.mixamorigHips} />
            <skinnedMesh name="Cube002" geometry={nodes.Cube002.geometry} material={materials['Simple Crystal']} skeleton={nodes.Cube002.skeleton}>
          ...
          <group name="base" position={[0.03, -0.55, -0.01]} rotation={[Math.PI / 2, 0, 0]} scale={0.71}>
            <mesh name="Ellipse" geometry={nodes.Ellipse.geometry} material={materials['Laminate floor']}>
              <MeshTransmissionMaterial
                roughness={0.1}
                metalness={0.8}
                envMap={ManTexture}
            ...
            <mesh name="Ellipse_1" geometry={nodes.Ellipse_1.geometry} material={materials['Special Metallic Car Paint']}>
              <MeshTransmissionMaterial
                roughness={0.4}
                metalness={0.2}
                envMap={ManTexture}
...
function VideoMaterial({ url }) {
  ...
  return <meshBasicMaterial 
   map={texture} transparent toneMapped={true} />
}
